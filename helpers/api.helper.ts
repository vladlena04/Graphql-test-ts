...
export class GraphQLService {
  ...
  async getAllTeas() {
    ...
    const query = "{ teas { id, name, description, price} }";
    const data = {
      query: query
    };
  ...
  async addNewTea(newTea: any, teaPrice: any, teaDescription: any) {
    const api = await request.newContext();
    const query = `mutation { addTea(teaInput: { name: "${newTea}", description: 
    "${teaDescription}", price: ${teaPrice}, 
    producerId: "6422eb079fc7ec0008c56f3f" }){ name, price } }`;
    const data = {
      query: query
    };
