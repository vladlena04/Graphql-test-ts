...
const newTea = faker.hacker.adjective();
const teaPrice = faker.finance.amount();
const teaDescription = faker.date.weekday() + ' ' + faker.commerce.product()
let teaID: string;
...
test.describe.serial('GRAPHQL DEMO', async () => {
    ...
    test('should add new tea', async () => {
        await graphQlService.addNewTea(newTea, teaPrice, teaDescription);
    });
